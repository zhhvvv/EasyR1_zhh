...
def accuracy_reward(response: str, ground_truth: str) -> float:
    ...
    return 1.0 if response_clean == ground_truth_clean else 0.0
...
def compute_score(
    ...
    Args:
        ...
        confidence_weight: 置信度奖励的权重
        low_entropy_threshold: 低熵阈值
        high_entropy_threshold: 高熵阈值
        high_confidence_threshold: 高置信度阈值
    
    修改点：
    - 删除format reward和format_weight
    - 简化accuracy判断，直接比较输出和ground truth
    - 权重只有accuracy_weight和confidence_weight
    """
    accuracy_weight = 1.0 - confidence_weight
    
    if DEBUG:
        print("\n" + "="*80)
        print("DEBUG: compute_score called")
        print(f"Batch size: {len(reward_inputs)}")
        print(f"Weights: accuracy={accuracy_weight:.3f}, confidence={confidence_weight:.3f}")
    ...
    for idx, reward_input in enumerate(reward_inputs):
        ...
        # 计算总分（不再包含format）
        base_score = accuracy_weight * accuracy_score
        overall_score = base_score + confidence_weight * conf_reward
        overall_score = max(0.0, min(1.0, overall_score))
        
        if DEBUG and idx < DEBUG_SAMPLE_COUNT:
            print(f"Overall score: {overall_score:.4f} = {accuracy_weight:.2f}*{accuracy_score:.2f} + {confidence_weight:.2f}*{conf_reward:.2f}")
